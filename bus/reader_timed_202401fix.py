...
#202401 only or until LTA fixes
specials = specials * 2
weekdays = weekdays * 2
...
df1 = pd.pivot_table(df1, index=['PT_CODE', 'DAY_TYPE'], columns=["TIME_PER_HOUR"], aggfunc={'TOTAL_TAP_IN_VOLUME': np.sum, 'TOTAL_USAGE': np.sum, 'TOTAL_TAP_OUT_VOLUME': np.sum}, fill_value=0)
